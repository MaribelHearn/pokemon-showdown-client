...
export class BattleScene implements BattleSceneStub {
	...
	setBgm(bgmNum: number) {
		...
		case -3:
			...
			break;
		case -4:
			this.bgm = BattleSound.loadBgm('audio/bgm/encounter.mp3', 48900, 109335, this.bgm);
			break;
		case -5:
			this.bgm = BattleSound.loadBgm('audio/bgm/turnabout.mp3', 7000, 84000, this.bgm);
			break;
		case -6:
			this.bgm = BattleSound.loadBgm('audio/bgm/never-gonna-give-you-up.mp3', 43000, 160750, this.bgm);
			break;
		case -7:
			this.bgm = BattleSound.loadBgm('audio/bgm/duckroll.mp3', 18570, 60800, this.bgm);
			break;
		case 1:
		...
		case 2:
			...
			break;
		case 3:
			this.bgm = BattleSound.loadBgm('audio/bgm/t-fanger.mp3', 8200, 85900, this.bgm);
			break;
		case 4:
			this.bgm = BattleSound.loadBgm('audio/bgm/doomsday.mp3', 160, 43830, this.bgm);
			break;
		case 5:
		...
		case 7:
			...
			break;
		case 8:
			this.bgm = BattleSound.loadBgm('audio/bgm/smithy.mp3', 6000, 72000, this.bgm);
			break;
		case 9:
		...
		case 13:
			...
			break;
		case 14:
			this.bgm = BattleSound.loadBgm('audio/bgm/vertigo.mp3', 7000, 131850, this.bgm);
			break;
		case 15:
			this.bgm = BattleSound.loadBgm('audio/bgm/trolololo.mp3', 2000, 163000, this.bgm);
			break;
		case 16:
			this.bgm = BattleSound.loadBgm('audio/bgm/windows-error-remix.mp3', 25500, 277500, this.bgm);
			break;
		case 17:
			this.bgm = BattleSound.loadBgm('audio/bgm/tabuu-theme.mp3', 1000, 188000, this.bgm);
			break;
		case 18:
			this.bgm = BattleSound.loadBgm('audio/bgm/decisive-magic-battle.mp3', 270, 126600, this.bgm);
			break;
		case 19:
			this.bgm = BattleSound.loadBgm('audio/bgm/marisa.mp3', 1500, 117900, this.bgm);
			break;
		case 20:
			this.bgm = BattleSound.loadBgm('audio/bgm/sanae.mp3', 13600, 177080, this.bgm);
			break;
		case 21:
	...
	updateBgm() {
		...
		if (nowPlaying) {
			...
			if (this.bgm !== null && !this.bgmPosted) {
				const theme = this.bgm.url.replace(/(\.mp3|audio\/|bgm\/)/g, '');
				this.log.addDiv('chat battle-history', '<small>🎵 <em class="bgm">' + bgmName[theme] + '</em></small>');
				this.bgmPosted = true;
			}
		} else if (this.bgm) {
			this.bgm.pause();
...
export class PokemonSprite extends Sprite {
	...
	animSummon(pokemon: Pokemon, slot: number, instant?: boolean) {
		...
		if (this.cryurl && pokemon.speciesForme !== 'Chuck Testa' && pokemon.speciesForme !== 'Snake') {
	...
	animFaint(pokemon: Pokemon) {
		...
		if (this.cryurl) {
			...
			const digitalFaintCry = ['Blue Screen', 'Internet Explorer', 'Inori Aizawa', 'Microsoft Edge', 'Mozilla Firefox', 'Google Chrome', 'Recycle Bin'];
	...
	updateStatbar(pokemon: Pokemon, updatePrevhp?: boolean, updateHp?: boolean, tmTrainer?: boolean) {
		...
		if (tmTrainer === false || pokemon.status === 'tmt' && tmTrainer === true) {
			this.resetStatbar(pokemon);
			return;
		}
		if (!this.$statbar) {
...
BattleStatusAnims['warlockpunch'] = {anim: BattleStatusAnims['flinch'].anim};
