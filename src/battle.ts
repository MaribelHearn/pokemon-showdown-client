...
export class Battle {
	...
	animateMove(pokemon: Pokemon, move: Move, target: Pokemon | null, kwArgs: KWArgs) {
		if (!this.seeking) {
			...
			else if (move.num >= 2000 && !['objection', 'holdit', 'takethat', 'nope', 'dinnerblaster'].includes(move.id)) {
				BattleSound.playEffect(`audio/moves/${move.id}.mp3`);
			}
		}
	...
	runMinor(args: Args, kwArgs: KWArgs, nextArgs?: Args, nextKwargs?: KWArgs) {
		...
		case '-crit': {
			...
			let frompoke = this.getPokemon(args[2])!;
			const species = frompoke.speciesForme;
		...
		case '-status': {
			...
			case 'tmt':
				this.scene.resultAnim(poke, 'Burned', 'brn');
				this.scene.runStatusAnim('brn' as ID, [poke]);
				this.scene.updateStatbar(poke, false, false, true);
				break;
			case 'tox':
		...
		case 'detailschange': {
			...
			this.scene.animTransform(poke, true, true, !!this.seeking);
			if (poke.status === 'tmt') {
				args.push('TMTRAINER');
			}
			this.log(args, kwArgs);
