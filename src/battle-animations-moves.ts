...
BattleMoveAnims['washingmachine'] = {
	anim(scene, [attacker, defender]) {
		// first loop
		for (let i = 0; i < 10; i++) {
			scene.showEffect('waterwisp', {
...
BattleMoveAnims['perfectburn'] = {
	anim(scene, [attacker, defender]) {
		scene.backgroundEffect(`url('${Dex.fxPrefix}/bg-volcanocave.png')`, 1000, 0.6);
		scene.showEffect('fireball', {
			x: defender.x,
			y: defender.y,
			z: defender.z,
...
BattleMoveAnims['immafirinmahlazer'] = {
	anim(scene, [attacker, defender]) {
		...
		scene.backgroundEffect('#000000', 1500, 0.6);
...
BattleMoveAnims['lightcatnarok'] = {anim:BattleMoveAnims['explosion'].anim};
...
BattleMoveAnims['massivedamage'] = {anim: BattleMoveAnims['stompingtantrum'].anim};
BattleMoveAnims['houraijewel'] = {anim: BattleMoveAnims['energyball'].anim};
