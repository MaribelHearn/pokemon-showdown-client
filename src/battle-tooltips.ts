...
class BattleTooltips {
	...
	showMoveTooltip(move: Move, isZOrMax: string, pokemon: Pokemon, serverPokemon: ServerPokemon, gmaxMove?: Move) {
		...
		if (this.battle.hardcoreMode) {
			...
			if ((move.type === 'Fire' || move.type === 'Water' || move.type === 'Electric' || move.type === 'Ground') && move.category !== 'Status' && ability === 'elementalist') {
		...
		if (!illusionIndex || illusionIndex === 1) {
			...
			if (clientPokemon?.volatiles.formechange) {
				if (clientPokemon.volatiles.transform) {
					...
					if (clientPokemon.speciesForme === 'smithytankhead' && clientPokemon.shiny) {
						text += `<small>(Changed forme: Smithy-Mask Head)</small><br />`;
					}
					else if (clientPokemon.speciesForme === 'smithymagichead' && clientPokemon.shiny) {
	...
	getPPUseText(moveTrackRow: [string, number], showKnown?: boolean) {
		...
		if (ppUsed != null || moveName.charAt(0) === '*') {
	...
	getMoveType(move: Move, value: ModifiableValue, forMaxMove?: boolean | Move): [TypeName, 'Physical' | 'Special' | 'Status'] {
		...
		if (allowTypeOverride && category !== 'Status' && !move.isZ && !move.id.startsWith('hiddenpower')) {
			if (moveType === 'Normal') {
				...
				if (value.abilityModify(0, 'Corruption')) moveType = '???';
			}
	...
	getMoveBasePower(move: Move, moveType: TypeName, value: ModifiableValue, target: Pokemon | null = null) {
		...
		if (move.id === 'terrainpulse' && pokemon.isGrounded(serverPokemon)) {
			if (
				...
				this.battle.hasPseudoWeather('Glitchy Terrain')
			) {
		...
		if (
			...
			if (move.type === 'Normal') {
				...
				value.abilityModify(this.battle.gen > 6 ? 1.2 : 1.3, "Conflagrate");
				value.abilityModify(this.battle.gen > 6 ? 1.2 : 1.3, "Corruption");
			}
			if (this.battle.gen > 6) {
		...
		if ((this.battle.hasPseudoWeather('Electric Terrain') && moveType === 'Electric') ||
			...
			(this.battle.hasPseudoWeather('Glitchy Terrain') && moveType === '???')) {
