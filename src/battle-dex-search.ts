...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	constructor(searchType: T, format = '' as ID, speciesOrSet: ID | PokemonSet = '' as ID) {
		...
		if (format.includes('vgc')) this.formatType = 'doubles';
...
class BattlePokemonSearch extends BattleTypedSearch<'pokemon'> {
	...
	getBaseResults(): SearchRow[] {
		...
		if ((format.endsWith('cap') || format.endsWith('caplc')) && dex.gen < 8) {
			...
			this.formatType !== 'letsgo' && this.formatType !== 'bdspdoubles' && this.formatType !== 'dlc1doubles' && !this.formatType?.startsWith('fundex') &&
			(
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	private moveIsNotUseless(id: ID, species: Species, abilityid: ID, itemid: ID, moves: string[]) {
		...
		case 'hiddenpowerice':
			return !moves.includes('icebeam') && (dex.gen > 3 || !moves.includes('icepunch')) || (dex.gen < 4 || !moves.includes('aurorabeam'));
		...
		case 'hyperbeam':
			// Fundex: Hyper Beam not useless for Cell and Octogonapus
			return species.baseSpecies === 'Cell' || species.baseSpecies === 'Dr. Octogonapus';
		case 'hypnosis':
		...
		case 'relicsong':
			return species.id === 'meloetta' || species.id === 'corruptedwake';
		case 'refresh':
		...
		case 'skyuppercut':
			return !moves.includes('closecombat') && !moves.includes('jumpkick') && !moves.includes('highjumpkick') && !moves.includes('sacredsword') && !moves.includes('thunderouskick');
		case 'smackdown':
		...
		if (this.formatType?.includes('doubles') && BattleMoveSearch.GOOD_DOUBLES_MOVES.includes(id)) {
			return true;
		}
	...
	// Fundex: add custom good status moves
	static readonly GOOD_STATUS_MOVES = [
		'acidarmor', 'agility', 'aromatherapy', 'auroraveil', 'autotomize', 'banefulbunker', 'barrelroll', 'batonpass', 'bellydrum', 'blueufosofgrief', 'bulkup', 'cakelie', 'calmmind', 'clangoroussoul', 'coil', 'cottonguard', 'courtchange', 'curse', 'defog', 'destinybond', 'detect', 'disable', 'dragondance', 'drainingkiss', 'dreaminduction', 'electricalshield', 'encore', 'extremeevoboost', 'firewall', 'geomancy', 'glare', 'glitchyterrain', 'goldencookie', 'greenufosofloyalty', 'hardreset', 'haze', 'healbell', 'healingwish', 'healorder', 'heartswap', 'honeclaws', 'houraielixir', 'kingsshield', 'leechseed', 'lightscreen', 'lovelykiss', 'lunardance', 'magiccoat', 'maxguard', 'memento', 'milkdrink', 'miraclefruit', 'moonlight', 'morningsun', 'mysterioussong', 'nastyplot', 'naturesmadness', 'noretreat', 'obstruct', 'painsplit', 'partingshot', 'patriotselixir', 'perishsong', 'protect', 'quiverdance', 'reboot', 'recollection', 'recover', 'redufosofrage', 'reflect', 'reflecttype', 'releaseoftheid', 'rest', 'roar', 'rockpolish', 'roost', 'shellsmash', 'shiftgear', 'shoreup', 'slackoff', 'sleeppowder', 'sleeptalk', 'softboiled', 'softreset', 'spikes', 'spikyshield', 'spore', 'stealthrock', 'stickyweb', 'strengthenjutsu', 'strengthsap', 'study', 'substitute', 'switcheroo', 'swordsdance', 'synthesis', 'tailglow', 'tailwind', 'taunt', 'theworld', 'thunderwave', 'toxic', 'toxicspikes', 'transform', 'trick', 'umadbro', 'whirlwind', 'willowisp', 'wish', 'yawn', 'pixiedust',
	] as ID[] as readonly ID[];
	// Fundex: add custom good weak moves
	static readonly GOOD_WEAK_MOVES = [
		'accelerock', 'acidbomb', 'acrobatics', 'aquajet', 'avalanche', 'bonemerang', 'bouncybubble', 'bulletpunch', 'bulletspray', 'bullseye', 'buzzybuzz', 'circlethrow', 'clearsmog', 'doubleironbash', 'dragondarts', 'dragontail', 'endeavor', 'evilsealingcircle', 'facade', 'fantasyseal', 'firefang', 'flightofidaten', 'flipturn', 'freezedry', 'frustration', 'geargrind', 'grassknot', 'gyroball', 'hex', 'holdit', 'icefang', 'iceshard', 'iciclespear', 'knockoff', 'koin3steps', 'lemmingsparade', 'lightarrow', 'lowkick', 'machpunch', 'mysteriouspower', 'narrowconfines', 'needlestorm', 'nightshade', 'nuzzle', 'nyan', 'objection', 'painflow', 'paralyzer', 'parasoltwirl', 'pikapapow', 'pingas', 'psychocut', 'pursuit', 'quickattack', 'rapidspin', 'return', 'rockblast', 'scorchingsands', 'seismictoss', 'shadowclaw', 'shadowsneak', 'sizzlyslide', 'sovietreversal', 'storedpower', 'stormthrow', 'strawberrycross', 'suckerpunch', 'superfang', 'superglitch', 'superjump', 'surgingstrikes', 'surprise', 'suzuranpoison', 'tailslap', 'thousandcuts', 'tripleaxel', 'umadbro', 'uturn', 'veeveevolley', 'voltswitch', 'watershuriken', 'weatherball', 'whirlingfortress', 'zerolaser', 'zzzap',
	] as ID[] as readonly ID[];
	static readonly BAD_STRONG_MOVES = [
		'beakblast', 'belch', 'burnup', 'crushclaw', 'dragonrush', 'dreameater', 'eggbomb', 'firepledge', 'flyingpress', 'grasspledge', 'hyperbeam', 'hyperfang', 'hyperspacehole', 'jawlock', 'landswrath', 'lastresort', 'megakick', 'megapunch', 'mistyexplosion', 'muddywater', 'nightdaze', 'pollenpuff', 'rockclimb', 'selfdestruct', 'shelltrap', 'skyuppercut', 'slam', 'strength', 'submission', 'synchronoise', 'takedown', 'thrash', 'uproar', 'waterpledge',
	] as ID[] as readonly ID[];
