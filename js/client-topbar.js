(function ($) {
	...
	var CustomBackgroundPopup = this.CustomBackgroundPopup = Popup.extend({
		...
		initialize: function () {
			...
			buf += '<button name="setBg" value="catnarok"' + (cur === 'catnarok' ? ' class="cur"' : '') + '><span class="bg" style="background-position:0 -' + (90 * 0) + 'px"></span>Catnarok</button>';
			buf += '<button name="setBg" value="hakurei"' + (cur === 'hakurei' ? ' class="cur"' : '') + '><span class="bg" style="background-position:0 -' + (90 * 1) + 'px"></span>Hakurei Shrine</button>';
